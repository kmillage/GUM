...
run_post_prm_pt_cmsy<- function(dat, b_to_k_ratio = 0.4, res = 'Medium',start_bio = NA, mid_bio = NA, final_bio = NA,CommonError = 0.05, sigR = 0, Smooth = F, Display = F,n = 10000,NumCPUs = 1)
{
  ...
  ct   <- dat$catch #(dat$Catch[(dat[,IdVar])==stock])  ## assumes that catch is given in tonnes, transforms to 1'000 tonnes

  bio<- pmin(1,(dat$BvBmsy*dat$b_to_k_ratio)) #pull out bvbmsy (transposed to B/K)
  ...
  if (sum(ct,na.rm=T)>0 & length(LastCatchYear)>0 & length(ct)>1)
  {
    ...
    start_bio    <- pmin(1,pmax(0,c(qnorm(0.45,bio[1],bioerror[1]),qnorm(0.55,bio[1],bioerror[1]))))
    ...
    mid_bio    <- NA # pmin(1,pmax(0,c(qnorm(0.45,bio[1],bioerror[1]),qnorm(0.55,bio[1],bioerror[1]))))
    ...
    final_bio    <- pmin(1,pmax(0,c(qnorm(0.45,bio[nyr],bioerror[nyr]),qnorm(0.55,bio[nyr],bioerror[nyr]))))
    ...
    if(length(g1)>=10) {
      ...
      max_k1b  <- max(k1[(g1*k1)*b_to_k_ratio <mean_msy1],na.rm=T) ## largest k1 that gives mean MSY
      max_k1 <- if(max_k1a < max_k1b) {max_k1a} else {max_k1b}
      ...
      {
        par(mfcol=c(2,3))
        plot(yr, ct, type="l", ylim = c(0, max(ct)), xlab = "Year", ylab = "Catch (MT)", main = unique(dat$id))
